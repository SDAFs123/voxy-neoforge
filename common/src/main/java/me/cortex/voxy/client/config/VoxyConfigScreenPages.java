    ...
    public static OptionPage page() {
        ...
        groups.add(OptionGroup.createBuilder()
                .add(OptionImpl.createBuilder(boolean.class, storage)
                        ...
                        .setControl(opt -> new SliderControl(opt, 16, 512, 1, v -> Component.literal(v + " chunks")))
                        .setBinding((s, v) -> {
        ...
        groups.add(OptionGroup.createBuilder()
                .add(OptionImpl.createBuilder(boolean.class, storage)
                        ...
                        .setBinding((s, v) -> s.requestDistance = v, s -> s.requestDistance)
                        .setImpact(OptionImpact.HIGH)
                        .build()
                ).build()
        );
        ...
        groups.add(OptionGroup.createBuilder()
                .add(OptionImpl.createBuilder(boolean.class, storage)
                        .setName(Component.translatable("voxy.config.debug.suppress_warnings"))
                        .setTooltip(Component.translatable("voxy.config.debug.suppress_warnings.tooltip"))
                        .setControl(TickBoxControl::new)
                        .setBinding((s, v) -> {
                            s.suppressInitWarnings = v;
                            Logger.SUPPRESS_INIT_WARNINGS = v;
                        }, s -> s.suppressInitWarnings)
                        .setImpact(OptionImpact.LOW)
                        .build()
                ).build()
        );
