...
public class VoxyClientInstance extends VoxyInstance {
    ...
    public VoxyClientInstance() {
        super();
        Logger.SUPPRESS_INIT_WARNINGS = VoxyConfig.CONFIG.suppressInitWarnings;
        var path = getFlashbackStoragePath();
    ...
    private static Path getFlashbackStoragePath() {
        if (!Platform.isModLoaded("flashback")) {
            return null;
        }
        try {
            var flashbackClass = Class.forName("me.cortex.voxy.client.compat.FlashbackCompat");
            var getMethod = flashbackClass.getMethod("getReplayStoragePath");
            return (Path) getMethod.invoke(null);
        } catch (Exception e) {
            return null;
        }
    }
